...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (phoneHeaderBtn) {
    phoneHeaderBtn.addEventListener('click', function() {
      const phoneNumber = '+972545775998';
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async function(e) {
      ...
      if (!window.zappyContactFormLoaded) {
        console.log('⚠️ Zappy: Handler not properly initialized, skipping API call');
      } else {
        try {
          ...
          const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
            ...
            body: JSON.stringify({
              websiteId: '42717073-467e-484d-917a-2f9a062839b2',
              name: data.name || '',
              email: data.email || '',
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Psyformance משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
